...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true) {
        $this->options = array(
			...
			'database' => 'buet73dev',
			'host' => 'buet73dev.db.6539354.hostedresource.com',
			'username' => 'buet73dev',
			'password' => 'Alumni0!',
			// end
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']).'/profile/', //Asif
            'upload_url' => $this->get_full_url().'/profile/', //Asif
            'user_dirs' => true,
            'mkdir_mode' => 0755,
            'param_name' => 'files', //Asif
    ...
    protected function get_upload_path($file_name = null, $version = null) {
        ...
        //$file_name = $file_name ? "profile".substr($file_path, strrpos($file_path, ".")) : '';
	...
	{
		$oldimgrs = mysql_fetch_array($this->query("Select `profpic` from `users` Where `id`=".$this->get_user_id()));
		$oldimg = $oldimgrs['profpic'];
		$upath = $this->options['upload_dir'].$this->get_user_path();
		if ($oldimg!=""&&file_exists($upath."/".$oldimg)&&$oldimg!=$whichimg)
		{
			unlink($upath."/".$oldimg);
		}
		if ($oldimg!=""&&file_exists($upath."/thumbnail/".$oldimg&&$oldimg!=$whichimg))
		{
			unlink($upath."/thumbnail/".$oldimg);
		}
		$add_to_db = $this->query("UPDATE `users` SET `profpic`='".$whichimg."' Where `id`=".$this->get_user_id()) or die(mysql_error());
		return $add_to_db;
	}
	function delete_img($delimg)
	{
	$delete_from_db = $this->query("DELETE FROM `imgtest` WHERE `img` = '$delimg'") or die(mysql_error());
